  ...
  describe ".create" do
    ...
    context "when given bad/missing params" do
      it "throws an error when required params are missing" do
        ...
        expect {refund.create({})}.to raise_error(ArgumentError), "called Refund.create with empty object"
        expect {refund.create({card: ENV['TEST_CARD_ID']})}.to raise_error(ArgumentError), "called Refund.create with '{card: ENV['TEST_CARD_ID']}'"
        expect {refund.create([])}.to raise_error(ArgumentError), "called Refund.create with empty array"
      end
    end
